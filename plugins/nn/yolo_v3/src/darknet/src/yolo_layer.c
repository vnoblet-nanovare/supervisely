...
{
    box b;
    // printf("n=%d, index=%d, i=%d, j=%d, lw=%d, lh=%d, w=%d, h=%d, stride=%d\n",
    ...
    *(l.cost) = 0;
    for (b = 0; b < l.batch; ++b) {
        ...
        for(t = 0; t < l.max_boxes; ++t){
            ...
            i = (truth.x * l.w);
            j = (truth.y * l.h);
            // printf("i=%d, j=%d, truth.x=%lf, truth.y=%lf, l.w=%d, l.h=%d\n", i, j, truth.x, truth.y, l.w, l.h);
...
float find_max_score(layer l, int w, int h)
{
    int i,j,n;
    float max_sc = 0;
    ...
    for (i = 0; i < l.w*l.h; ++i){
        ...
        for(n = 0; n < l.n; ++n){
            ...
            if(objectness > max_sc) max_sc = objectness;
        }
    }
    return max_sc;
}
